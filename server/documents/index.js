...
module.exports = ({ pname, price, item, sr ,quantity,age, email, address, sr1 ,quantity1,price1,item1}) => {
...
return `
   ...
   <body>
      <header class="clearfix">
         ...
         <div><span>DATE</span> ${`${("0" + today.getDate()).slice(-2)} / ${("0" + (today.getMonth() + 1)).slice(-2)} / ${today.getFullYear()}.`}</div>
         <div><span>TIME</span> ${`${("0" + today.getHours()).slice(-2)} : ${("0" + today.getMinutes()).slice(-2)}: ${("0" + today.getSeconds()).slice(-2)}`}</div>
      ...
      <main>
         ...
         <tbody>
            ...
            <tr>
               <td class="service">${sr1}</td>
               <td class="desc">${item1}</td>
               <td class="service">${quantity1}</td>
               <td class="service">${parseFloat(price1).toFixed(2)}</td>
               <td class="total"> ${parseFloat(price1 * quantity1).toFixed(2)} </td>
            </tr>
            <tr>
               <td colspan="4" >SUBTOTAL</td>
               <td class="total"> ${parseFloat((price * quantity)+ (price1 * quantity1)).toFixed(2)}  </td>
            </tr>
         </tbody>
         </table>
         <div id="notices">
